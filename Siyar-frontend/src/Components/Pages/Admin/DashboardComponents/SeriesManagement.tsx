...
import { useGetSeriesQuery, useDeleteSeriesMutation } from '../../../../Redux/api/seriesApi';
...
const SeriesManagement: React.FC = () => {
  ...
  const { data: seriesResponse, isLoading, refetch } = useGetSeriesQuery({});
  ...
  const filteredSeries = series.filter((seriesItem) => {
    ...
    const matchesStatus = statusFilter === 'all' || 
      (statusFilter === 'active' && seriesItem.isActive) || 
      (statusFilter === 'inactive' && !seriesItem.isActive);
  ...
  const handleEditSeries = (seriesItem: ISeries) => {
    setModalMode('edit');
    setSelectedSeries(seriesItem);
    setShowModal(true);
  };

  const handleDeleteSeries = async (seriesItem: ISeries) => {
    // Show confirmation dialog with Swal
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: `Do you want to delete "${seriesItem.title}"? This action cannot be undone.`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await deleteSeries(seriesItem._id).unwrap();
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error!',
          text: 'Failed to delete series. Please try again.',
  ...
  const handleModalSuccess = () => {
    refetch(); // Refetch series after successful operation
  };
  ...
  const stats = {
    total: series.length,
    active: series.filter(s => s.isActive).length,
    inactive: series.filter(s => !s.isActive).length
  };
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-xl lg:text-2xl font-bold">Series Management</h2>
        <button 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4">
            <div className="form-control">
              <label className="label">
                <span className="label-text text-xs lg:text-sm">Search Series</span>
              </label>
      ...
      <div className="card bg-base-100 shadow-lg">
        <div className="card-body p-3 lg:p-6">
          <h3 className="card-title text-base lg:text-lg">
            Series ({filteredSeries.length} of {series.length})
          </h3>
            ...
            <>
              ...
              <div className="lg:hidden space-y-3">
                {filteredSeries.map((seriesItem) => (
                  <div key={seriesItem._id} className="card bg-base-200 p-3">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h4 className="font-semibold text-sm">{seriesItem.title}</h4>
                        <p className="text-xs opacity-70">ID: {seriesItem._id.slice(-8)}</p>
                        <div className="mt-2">
                          {getStatusBadge(seriesItem.isActive)}
                        </div>
              ...
              <div className="hidden lg:block overflow-x-auto">
                <table className="table table-zebra w-full">
                  ...
                  <tbody>
                    {filteredSeries.map((seriesItem) => (
                      <tr key={seriesItem._id}>
                        <td>
                          <div className="font-semibold">{seriesItem.title}</div>
                          <div className="text-sm opacity-70">ID: {seriesItem._id.slice(-8)}</div>
                        </td>
                        <td>{getStatusBadge(seriesItem.isActive)}</td>
                        <td>
                          <div className="text-sm">
                            {new Date(seriesItem.createdAt).toLocaleDateString()}
                          </div>
                          <div className="text-xs opacity-70">
                            {new Date(seriesItem.createdAt).toLocaleTimeString()}
                          </div>
                        </td>
                        <td>
